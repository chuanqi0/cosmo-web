app.controller('SwapFaceCtrl', ['$scope', '$cookieStore', function ($scope, $cookieStore) {
    ...
    $scope.generateWholeImageDataUrl = function () {
        ...
        var toLeft4 = (canvas.width - 20 * (canvas.width / 343) * (tag4.length));
        // tag4 = tag4.substr(0, 5);
        ctx.fillText(tag4 != '-' ? tag4 : '', toLeft4, 110 * (canvas.height / 412), 80 * (canvas.width / 343));
        // 文字5
        ctx.fillStyle = "#BA882A";
        var toLeft5 = (canvas.width - 20 * (canvas.width / 343) * (tag5.length));
        ...
        var toLeft6 = (canvas.width - 22 * (canvas.width / 343) * (tag6.length));
